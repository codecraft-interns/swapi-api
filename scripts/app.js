  ...
  for (var i in element) {
    for (var j = 0; j < local_data.length; j++) {
      if (i == local_data[j].id) {
        ...
        if (branchelement[side + '_' + groupid] > 3) { branchelement[side + '_' + groupid] = 3; }
...
function hidemember(id, side, count, groupid) {
  if (count > 1) {
...
front.addEventListener('click', e => {
  if (e.target.classList.contains('group_member_delete_left')) {
    ...
    hidemember(e.target.id, 'left', branchelement['left_' + e.target.previousElementSibling.id], e.target.previousElementSibling.id);
...
back.addEventListener('click', e => {
  if (e.target.classList.contains('group_member_delete_right')) {
    ...
    hidemember(e.target.id, 'right', branchelement['right_' + e.target.previousElementSibling.id], e.target.previousElementSibling.id);
